...
const ControlBar: React.FC<ControlBarProps> = ({ 
  isMuted, isVideoOn, isSharingScreen, isHandRaised, onToggleHand, onMute, onVideo, onShareScreen, onLeave, role, 
    ...
    <div className="h-20 bg-zinc-950 flex items-center justify-between px-4 md:px-8 shrink-0 border-t border-white/5 shadow-2xl z-30">
      ...
      <div className="flex-1 hidden md:flex items-center gap-6">
        <div className="flex flex-col">
          <label className="text-[8px] font-black text-zinc-600 uppercase tracking-widest mb-1 ml-1">Translation Bridge</label>
      ...
      <div className="flex items-center gap-2 md:gap-3 px-4 py-2 rounded-2xl bg-zinc-900/50 backdrop-blur-xl border border-white/5 shadow-inner">
        <ControlButton onClick={onMute} active={!isMuted} title={isMuted ? 'Unmute' : 'Mute'}>
          {isMuted ? (
        ...
        <ControlButton onClick={onVideo} active={isVideoOn} title={isVideoOn ? 'Cam Stop' : 'Cam Start'}>
        ...
        <ControlButton onClick={onToggleHand} active={isHandRaised} title="Hand">
           <span className="text-lg">{isHandRaised ? '✋' : '👋'}</span>
        </ControlButton>
        {role !== Role.OBSERVER && (
        ...
        <ControlButton onClick={onToggleSidebar} active={false} title="Toggle">
        ...
        <ControlButton onClick={onToggleCaptions} active={showCaptions} title="Captions">
      ...
      <div className="flex-1 flex justify-end">
        <button
          onClick={onLeave}
          className="px-5 py-2.5 bg-red-600/10 hover:bg-red-600 text-red-500 hover:text-white font-black text-[10px] rounded-xl border border-red-600/20 transition-all flex items-center gap-2 uppercase tracking-widest"
        >
          Exit Class
        </button>
      </div>
